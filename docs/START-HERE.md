...
- ⏳ **Next: `ce-work` U2 — store + persistence** (`src/store/*`): `bun:sqlite` + Drizzle behind a repository interface, `PRAGMA journal_mode=WAL`, the self-recording access-log for the shared-brain hit rate (KTD3). **Branch off fresh `main`** (slice-1 is merged). Phase A order after: **U2 → U3 → U4 → U5 → U6 → U7 → U15**. KTDs 1–9 + the 4 hooks (#13) are build constraints — don't re-litigate. **Watch:** re-check trail-sufficiency on a messier mid-implementation resume during U5 (test-#2 caveat); the two deferred config-write findings live in `tasks/open-findings.md`. Reference: `docs/ARCHITECTURE.md` (the whole-system map).
