...
- AE1. **Covers R5–R7, R13.** Given a blueprint reproducing the `agent-cost-tracker` run-1 map (12 rows: 8 copy, 1 compose, 3 new — decomposing to 1 config-merge, the `.cursor/mcp.json` registration, plus 2 scaffold READMEs), when `apply` runs against fixture homes, then the produced native surfaces match the hand-provisioned ones and every write has an undo entry in the run's batch.
...
- **KTD8 — Diff semantics are format-aware.** Config-format targets diff semantically on parsed values (presence semantics — a foreign-formatted live file whose parsed content already matches is a no-op, never a false "will change"); whole-file `text` targets diff by bytes; scaffold targets are excluded from drift. This is the presence-semantics learning applied to the read side. `apply` acts on the diff's verdict: semantic-noop rows are skipped outright and the engine is invoked only for create/overwrite/merge rows — so `plan`, `status`, and `apply` can never disagree, and a foreign-formatted-but-semantically-equal config is never reformat-churned.
...
- **KTD11 — Projects resolve through a known-projects registry, never a caller-supplied path.** `init` records the project's name and absolute blueprint root in a known-projects file under the OS data dir; the propose MCP tools resolve `project` strictly through that registry and return a typed refusal for unknown names. No code ever joins an MCP-supplied string into a filesystem path — the substrate's `project` stays a logical key, and the registry is the one bridge to disk. Closes both the arbitrary-path oracle and U7's implementability gap without pulling U11's per-project axis forward.
...
flowchart TB
  ...
  W1 --> RUNREC["undo-journal entries stamped with the run's batch id"]
  W2 --> RUNREC
...
U1 lands first. U2, U3, and U4 depend only on U1 and are parallel-eligible in any order. U5 needs all three. U6 and U7 both follow U5 (U7 also calls U1's front-gate directly) and may run in parallel. Each unit is one unit-loop cycle (one PR).
...
- **Approach:** Add `"text"` to `ConfigFormat`; the compiler's `assertNever` guard drives every switch site. For `text`: parse/serialize identity on raw bytes — returned verbatim via an early return that bypasses `serializeConfig`'s trailing-newline canonicalization — patch = full content, no-op = byte equality decided before any write, `deepMerge`/`removeConfigKeys` reject the format explicitly (loud error, not silent misbehavior). Export the whole-file primitive named in KTD1 (wraps the private publish path; `mergeConfig` structurally rejects string patches). `UndoEntry` gains the optional batch fields (KTD2) — absent on installer writes, recorded when a caller supplies them. Backup, 0600 modes, atomic temp+rename, journal entry, `created`-flag delete-on-undo all reuse existing internals unchanged.
...
- **Test scenarios:**
  ...
  - Edge: byte-identical rewrite → true no-op (no backup accumulation, no journal entry); mode preserved on overwrite; extension-less path with explicit `format: "text"` override works; a source ending in two trailing newlines round-trips byte-identically (canonicalization bypassed); journal entries without batch fields still validate (existing installer journals stay readable).
...
- **Approach:** Apply walks plan rows in manifest order, skipping semantic-noop rows outright (KTD8): config-merge via `mergeConfig`, copy/compose/scaffold via the whole-file primitive, every write stamped with this run's batch id (KTD2). A caught mid-batch error rolls back this batch's earlier writes via their undo ids (the installer ladder generalized to N files). `runs.ts` derives batches from the journal (group by run id, newest per project) and emits the report — shape mirrors `UninstallOutcome` (`applied / failed / warnings / noops`). Per-file `AppliedButUnjournaledError` → warn-and-continue, flagged un-undoable in the report. `undo` resolves the newest batch from the journal, reverses LIFO, reports superseded/refused entries per-file.
...
- **Test scenarios:**
  ...
  - Edge (Covers AE3): second apply with no edits → zero writes, empty batch, no backup accumulation; a foreign-formatted live config whose parsed content already matches gets zero writes on apply (semantic-noop skip, no reformat churn); applying the `.cursor/mcp.json` config-merge row against a fixture whose `mcp.json` already holds a foreign MCP server preserves that server (routed through the merge path, never whole-file); undo after a *second* apply refuses the older batch's superseded entries with a per-file "superseded by a later apply" report.
...
- **Verification:** AE1/AE3/AE7 pass; batch undo works in a fresh process (batches derived from the journal — no in-memory state needed); `bun test` green.
...
- **Test scenarios:**
  - Happy (Covers AE2): `init` + `apply` on an empty fixture project → all three harnesses role-ready; immediate `plan` reports nothing to change; `RUN_STATE.md` exists with seed content; the project appears in the known-projects registry.
...
- **Files:** `src/mcp/provision-tools.ts`, `src/mcp/server.ts`, `src/server/routes.ts`, `src/server/index.ts`, `tests/provision-mcp.test.ts`.
- **Approach:** `registerProvisionTools(server, deps)` alongside `registerBrainTools` (same one-function registration convention; the security gate stays upstream in `routes.ts`, unchanged — its deps-wiring site gains the new registration). `McpDeps` gains a project resolver backed by the known-projects registry (KTD11): `project` stays a logical key and is never path-joined. Two tools: `provision_status(project)` → drift summary; `provision_plan(project)` → advisory plan rows. Both resolve through the registry (unknown name → typed refusal, no filesystem access), then run the U1 front-gate; absent/invalid/secret-hit → typed refusal, never a partial plan; output carries paths/actions/drift only, never file contents (KTD7). Best-effort access-log per the existing tool pattern.
...
- **Test scenarios:**
  ...
  - Error: a `project` not in the known-projects registry → typed refusal with zero filesystem access; absent blueprint → typed refusal; schema-incompatible blueprint → refusal carrying the R1 upgrade/migrate message; secret-tripping blueprint → refusal with file named, zero content echoed.
...
| Acceptance | AE1–AE7 each traced to a named passing test | U4, U5, U6, U7 |
