...
- ⏳ **Next: `ce-work` U14 — config-write discipline engine** (`src/configwrite/engine.ts` + `undo.ts`): format-aware merge (JSON/TOML/YAML) behind one API; timestamped backup → merge-don't-clobber → undo journal; write-to-temp + atomic rename; backups `0600` under the OS data dir. The shared safety utility every installer (U6/U8) + provision action (U10) uses — KTD6 / R11. On branch `feat/slice-1-substrate` (not pushed). Phase A order after: **U14 → U2 → U3 → U4 → U5 → U6 → U7 → U15**. KTDs 1–9 + the 4 hooks (#13) are build constraints — don't re-litigate. **Watch:** re-check trail-sufficiency on a messier mid-implementation resume during U5 (test-#2 caveat). Reference: `docs/ARCHITECTURE.md` (the whole-system map).
